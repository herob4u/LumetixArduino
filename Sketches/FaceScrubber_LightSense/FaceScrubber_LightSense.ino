...
//    }
    ...
    // Contrived message to simulate what a real message would look like. Copy paste the result of this write into the serial monitor and send it back to the arduino.
    byte msg[16+2] = {START_BYTE, 80, 80, 80, 80, 80,80, 80, 80, 80, 80,80, 80, 80, 80, 80,80, STOP_BYTE};
    Serial.write(msg, 18);
    delay(1);

    /* Testing code: turns on ALL leds of the first board to some desired value.
     *  If you want to do the same for other boards. Just index the tlcmanager.
     *  i.e for the top board: tlcmanager[0].<function>
     *  for the right board: tlcmanager[1].<function>
     *  for the bottom board: tlcmanager[2].<function>
     *  and so on.
     *  
     *  If you try to index a 3rd board when none is physically connected, your program should crash.
    */
    /*
    for(uint8_t i = 0; i < CHANNELS_COUNT; i++)
    {
        tlcmanager[0].pwm(i, 32);
    }*/
...
//        {
//            Serial.read(); // Reads the START_BYTE
...
//  //Serial.print("Gf/Bf: "); Serial.println(GBf);
  ...
  Serial.print("Temp delta: "); Serial.println(delta);

  if(delta < 0)
  {
      Light(yellow_channels, sizeof(yellow_channels), -delta);
      Light(white_channels, sizeof(yellow_channels), 0);
  }
  else if (delta > 0)
  {
      Light(white_channels, sizeof(yellow_channels), delta);
